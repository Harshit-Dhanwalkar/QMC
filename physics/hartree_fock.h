 ...
 * e = 4 * \pi * eps0 = 1)
 *
 * Only s-orbital: For central potential, e^--e^- interaction 1/|r-r'|
 * is expanded in Legendre multipoles Y^L. Angular (Gaunt-coefficient) selection
 * rules restrict which L actually contribute to direct (J) and exchange
 * (K) integrals between two orbitals of angular momenta l_a, l_b. For l_a = l_b
 * = 0 only surviving multipole is L = 0, so both J and K collapse onto same
 * radial kernel 1/max(r,r'), and exchange operator becomes an exact, closed
 * radial expression whihc discretize directly.
 *
 * MODEL:
 * n_orbitals distinct radial s-orbitals u_1(r) < u_2(r) < ... (u = r*R,
 * normalized: integral u_k(r)^2 dr = 1), each 'doubly occupied' (Aufbau,
 * spin up + spin down), i.e. this models atoms/ions with e^- count
 * = 2 * n_orbitals and all e^-s in s subshells (He: n_orbitals=1,
 * Be: n_orbitals=2, ...).
 *
 ...
 * where eps_k are converged orbital (Fock) eigenvalues and J_ij, K_ij
 * are direct/exchange two-e^-s integrals between orbitals i, j.
 */
