...
perturb_result_t perturb_nondeg(const eigen_t *unperturbed, int state_index,
    ...
    if (!unperturbed || !V_pert || state_index < 0 || state_index >= unperturbed->n) return res;
    int n = unperturbed->n;
    ...
    for (int k = 0; k < n; k++) {
        ...
        if (fabs(denom) < tol) continue;  // degeneracy not handled here
...
eigen_t *perturb_degenerate(const double *energies, const cmatrix_t *V_pert,
    ...
    if (!energies || !V_pert || !deg_indices || deg_size < 1) return NULL;
