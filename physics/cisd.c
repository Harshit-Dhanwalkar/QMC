#include "cisd.h"
#include "../core/linalg/complex_eigh.h"
#include "../core/matrix.h"
#include "second_quant.h"
#include <stdlib.h>

/* NOTE: Takes an already-built full Fock-space Hamiltonian (dimension full_dim
 * = 2^n_modes), restricts to reference determinant (spin-orbitals
 * 0..n_electrons-1 occupied) plus all single and double excitations out of it,
 * and diagonalizes that subspace.
 */
static cisd_result_t *cisd_solve_from_full_hamiltonian(cmatrix_t *H,
                                                       int n_modes,
                                                       int n_electrons,
                                                       int n_spatial_report) {
  if (!H) {
    return NULL;
  }

  if (n_electrons < 0 || n_electrons > n_modes) {
    cmatrix_free(H);

    return NULL;
  }

  int full_dim = H->nrows;
  /* NOTE: second_quant.h convention: spin-orbital 0 is the MSB (leftmost) bit
   * of Fock-space index (qstate_*'s qubit-0-is-leftmost convention), so
   * occupying spin-orbitals 0..n_electrons-1 means setting TOP n_electrons bits
   * of an n_modes-bit index, not the bottom ones. */
  unsigned reference =
      (n_electrons == 0)
          ? 0u
          : (unsigned)(((1u << n_electrons) - 1u) << (n_modes - n_electrons));

  /* First pass: count qualifying determinants (reference itself, plus
   * single/double excitations out of it), so it can be allocated exactly. */
  int dim = 0;
  for (int state = 0; state < full_dim; state++) {
    if (__builtin_popcount((unsigned)state) != n_electrons) {
      continue;
    }

    int excitation_weight = __builtin_popcount((unsigned)state ^ reference);
    if (excitation_weight == 0 || excitation_weight == 2 ||
        excitation_weight == 4) {
      dim++;
    }
  }

  int *basis_states = (dim > 0) ? malloc((size_t)dim * sizeof(int)) : NULL;
  if (!basis_states) {
    cmatrix_free(H);

    return NULL;
  }

  int count = 0;
  int reference_row = -1;
  for (int state = 0; state < full_dim; state++) {
    if (__builtin_popcount((unsigned)state) != n_electrons) {
      continue;
    }

    int excitation_weight = __builtin_popcount((unsigned)state ^ reference);
    if (excitation_weight == 0 || excitation_weight == 2 ||
        excitation_weight == 4) {
      if ((unsigned)state == reference) {
        reference_row = count;
      }

      basis_states[count++] = state;
    }
  }

  // count must equal dim by construction
  if (count != dim || reference_row < 0) {
    free(basis_states);
    cmatrix_free(H);

    return NULL;
  }

  cmatrix_t *H_sector = cmatrix_alloc(dim, dim);
  if (!H_sector) {
    free(basis_states);
    cmatrix_free(H);

    return NULL;
  }

  for (int i = 0; i < dim; i++) {
    for (int j = 0; j < dim; j++) {
      CMAT(H_sector, i, j) = CMAT(H, basis_states[i], basis_states[j]);
    }
  }

  double reference_energy = CMAT(H, (int)reference, (int)reference).re;

  cmatrix_free(H);

  eigen_t *eig = cmatrix_eigh_complex(H_sector);
  cmatrix_free(H_sector);
  if (!eig) {
    free(basis_states);

    return NULL;
  }

  cisd_result_t *res = malloc(sizeof(cisd_result_t));
  if (!res) {
    free(basis_states);
    eigen_free(eig);

    return NULL;
  }

  res->dim = dim;
  res->n_spatial = n_spatial_report;
  res->n_electrons = n_electrons;
  res->basis_states = basis_states;
  res->eigenvalues = eig->eigenvalues;   /* transfer ownership */
  res->eigenvectors = eig->eigenvectors; /* transfer ownership */
  res->ground_energy = dim > 0 ? eig->eigenvalues[0] : 0.0;
  res->reference_energy = reference_energy;
  res->correlation_energy = res->ground_energy - reference_energy;

  free(eig); // frees only eigen_t wrapper, not arrays

  return res;
}

cisd_result_t *cisd_solve(int n_spatial, const double *h_mo,
                          const double *eri_mo, double nuclear_repulsion,
                          int n_electrons) {
  if (n_spatial < 1 || !h_mo || !eri_mo) {
    return NULL;
  }

  int n_modes = 2 * n_spatial;
  if (n_electrons < 0 || n_electrons > n_modes) {
    return NULL;
  }

  cmatrix_t *H = second_quant_build_molecular_hamiltonian(
      n_spatial, h_mo, eri_mo, nuclear_repulsion);

  return cisd_solve_from_full_hamiltonian(H, n_modes, n_electrons, n_spatial);
}

cisd_result_t *cisd_solve_frozen_core(int n_spatial, int n_frozen,
                                      const double *h_mo, const double *eri_mo,
                                      double nuclear_repulsion,
                                      int n_electrons_active) {
  if (n_spatial < 1 || n_frozen < 0 || n_frozen >= n_spatial || !h_mo ||
      !eri_mo) {
    return NULL;
  }

  int n_active_spatial = n_spatial - n_frozen;
  int n_modes = 2 * n_active_spatial;
  if (n_electrons_active < 0 || n_electrons_active > n_modes) {
    return NULL;
  }

  cmatrix_t *H = second_quant_build_molecular_hamiltonian_frozen_core(
      n_spatial, n_frozen, h_mo, eri_mo, nuclear_repulsion);

  return cisd_solve_from_full_hamiltonian(H, n_modes, n_electrons_active,
                                          n_active_spatial);
}

void cisd_result_free(cisd_result_t *res) {
  if (!res) {
    return;
  }

  free(res->eigenvalues);
  cmatrix_free(res->eigenvectors);
  free(res->basis_states);
  free(res);
}
