...
molecular_dft_result_t *molecular_ks_lda(basis_function_t **basis, int n_basis,
  ...
  for (; iter < max_iter; iter++) {
    ...
    // Density on the grid, from the CURRENT (pre-update) density matrix
    for (int g = 0; g < ng; g++) {
      double v = 0.0;
    ...
    for (int g = 0; g < ng; g++) {
      ...
      /* Weight the energy/potential contributions by the TRUE (unfloored)
       * density, not the floored one, so an exactly-zero-density point
       * contributes exactly zero rather than a phantom floor-value times a
       * (possibly large, far-radial-shell) quadrature weight. */
