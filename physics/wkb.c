...
static const double WKB_EXP_FACTOR = 2.0;

/*
 * Helper compute \int_{x_min}^{x_max} \sqrt(2m * (E - V(x))) dx between turning
 * points. For a given E, find turning points x1, x2 where V(x) = E.
 */
...
double wkb_transmission(potential_fn V, void *params, double m, double hbar,
  ...
  return exp(-WKB_EXP_FACTOR * integral);
}
...
double wkb_tunneling_rectangular(double E, double V0, double width,
  ...
  return exp(-WKB_EXP_FACTOR * kappa * width);
}
