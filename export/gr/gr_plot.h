...
typedef struct {
    ...
    const char *color;
    double      line_width;
    int         tex_labels;   // 1 = render labels as TeX math
} gr_plot_opt_t;

/*
 * Plot y(x) using GR framework.
 */
int gr_plot_to_file(const char *filename, gr_format_t format,
 ...
 */
int gr_plot_lines(const char *filename, gr_format_t format,
                  const double *x, const double **ys, int n_series, int n_pts,
