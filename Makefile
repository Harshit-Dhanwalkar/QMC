...
EXAMPLES    = $(BUILD_DIR)/eg_01_particle_box \
              ...
              $(BUILD_DIR)/eg_60_uhf_geometry_optimization \
              $(BUILD_DIR)/eg_61_ump2 \
              $(BUILD_DIR)/eg_62_vqe_pauli_hamiltonian \
...
coverage: clean run-tests
	@if [ "$(LCOV_AVAIL)" = "no" ]; then \
		echo "lcov is not installed. Install lcov (e.g., 'sudo apt install lcov') to generate coverage reports." >&2; \
		exit 1; \
	fi
	@mkdir -p coverage
