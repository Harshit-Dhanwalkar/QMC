...
int main(void) {
  ...
  printf("   Harmonic oscillator V(x)=1/2x^2  (WKB is exact here):\n");
  ...
  /* T \approx \exp(-2*κ*L), \kappa=\sqrt(2*(V0-E)), L=1, E=1 ->
   * \kappa=\sqrt(2), T \approx \exp(-2\sqrt(2)) */
  double E = 1.0;
  double V0 = 2.0;
  double width = 1.0;
  double T_wkb = wkb_tunneling_rectangular(E, V0, width, 0.5);
  double kappa =
      sqrt(2.0 * (V0 - E) / 0.5); /* 2m(V-E)/ \hbar^2 with \hbar=1, m=1 */
