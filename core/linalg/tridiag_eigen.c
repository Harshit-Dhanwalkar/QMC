...
//   }
//
//   /* Apply reflection: A = (I - 2vv^\dagger/||v||^2)A(I -
//   2vv^\dagger/||v||^2)
//    */
//   for (int i = 0; i < m; i++) {
...
//       denominator = c_add(denominator, c_mul(c_conj(v->data[i]),
//       v->data[i]));
//     }
//     *lambda = c_div(numerator, denominator).re; // real part is the
//     eigenvalue
//
//     // Normalize w
//     cvector_normalize(w);
...
static double max_offdiag(cmatrix_t *A) {
  double max = 0.0;
  int n = A->nrows;
  for (int i = 0; i < n; i++) {
    for (int j = i + 1; j < n; j++) {
...
eigen_t *cmatrix_eigh(cmatrix_t *A) {
  ...
  // free(v);
  //
  // /* For higher states: use Ritz values or Lanczos method
  //  * For now, approximate with shifted power iteration */
  ...
  //     for (int i = 0; i < n; i++) {
  //       proj = c_add(
  //           proj, c_mul(c_conj(result->eigenvectors[j].data[i]),
  //           v->data[i]));
  //     }
  //     for (int i = 0; i < n; i++) {
  //       v->data[i] =
  ...
  //       for (int i = 0; i < n; i++) {
  //         v->data[i] =
  //             c_sub(v->data[i], c_mul(proj, CMAT(result->eigenvectors, i,
  //             j)));
  //       }
  //     }
  //     cvector_normalize(v);
