...
static const demo_entry_t demos[] = {
    ...
    {"build/eg_36_shor_code", "9-qubit Shor code: correcting arbitrary single-qubit errors (X, Y, Z)"},
    {"build/eg_37_quantum_algorithms", "Deutsch-Jozsa and Grover's search algorithm"},
    {"build/eg_38_second_quant", "Second quantization and Jordan-Wigner transformation"},
    {"build/eg_39_landau_levels", "Landau levels on a lattice via Peierls substitution (Hofstadter model)"},
    {"build/eg_40_openmp_qmc", "OpenMP-parallel VMC/DMC/PIMC replica runners (wall-clock speedup)"},
    {"build/eg_41_molecular_integrals", "General molecular integrals (McMurchie-Davidson): H2/STO-3G restricted Hartree-Fock"},
    {"build/eg_42_dft_atoms", "Kohn-Sham LDA DFT for atoms (He, Be), compared against Hartree-Fock"},
    {"build/eg_43_h2_vqe", "Real H2 VQE: AO integrals -> Jordan-Wigner -> variational quantum eigensolver"},
    {"build/eg_44_h4_vqe", "H4 chain: general N-basis RHF, FCI, and VQE at 8 qubits"},
    {"build/eg_45_lih_vqe", "LiH: p-orbitals, frozen-core reduction, and VQE at 10 qubits"},
    {"build/eg_46_noisy_vqe", "H2 Noisy VQE: T1/T2 decoherence and zero-noise extrapolation (ZNE)"},
    {"build/eg_47_geometry_optimization", "Molecular geometry optimization via HF/DFT energy gradients"},
    {"build/eg_48_ccsd", "Coupled-Cluster Singles and Doubles (CCSD)"},
    {"build/eg_49_ccsd_t", "Coupled-Cluster Singles, Doubles, and Triples CCSD(T)"},
    {"build/eg_50_general_basis_parser", "General Gaussian basis set parser and integral evaluator"},
    {"build/eg_51_ucc", "Unitary Coupled-Cluster (UCCSD) ansatz generation for VQE"},
    {"build/eg_52_qpe", "Quantum Phase Estimation (QPE) for molecular Hamiltonians"},
    {"build/eg_53_molecular_dft", "Kohn-Sham molecular DFT solver (LDA/VWN)"},
    {"build/eg_54_molecular_pbe_dft", "Molecular DFT with GGA functionals (PBE)"},
    {"build/eg_55_molecular_mp2", "General molecular MP2 (beyond s-orbitals): LiH with/without frozen core"},
    {"build/eg_56_schrodinger_solvers", "Comprehensive 1D/3D Schrödinger eigensolvers comparison"},
    {"build/eg_57_fci", "Full Configuration Interaction (FCI) exact diagonalization"},
    {"build/eg_58_spin_chain_dsf", "Dynamic structure factor for 1D Heisenberg spin chains"},
    {"build/eg_59_ising_chain", "Transverse-field Ising chain exact diagonalization and Jordan-Wigner"},
    {"build/eg_60_uhf_openshell", "Unrestricted Hartree-Fock: open-shell radicals (H, Li doublet, Li+)"},
    {"build/eg_61_uhf_geometry_optimization", "Analytic UHF nuclear gradient: open-shell geometry optimization (LiH+)"},
